...
static std::string handle_dump_weapons(const std::string&) {
    ...
    mora_skyrim_runtime::rt::for_each_form_of_type(0x29, weapon_collector, &weapons);
...
static std::string handle_dump_npcs(const std::string&) {
    ...
    mora_skyrim_runtime::rt::for_each_form_of_type(0x2B, npc_collector, &npcs);
...
static std::string handle_dump_armors(const std::string&) {
    ...
    mora_skyrim_runtime::rt::for_each_form_of_type(0x1A, armor_collector, &armors);
...
static std::string handle_dump_leveled_lists(const std::string&) {
    ...
    mora_skyrim_runtime::rt::for_each_form_of_type(0x2D, leveled_list_collector, &lists);
    mora_skyrim_runtime::rt::for_each_form_of_type(0x2C, leveled_list_collector, &lists);
...
static std::string handle_lookup(const std::string& cmd) {
    ...
    void* form = mora_skyrim_runtime::rt::lookup_form_by_id(formid);
    std::ostringstream ss;
    ss << R"({"ok":true,"formid":"0x)" << std::hex << std::setfill('0') << std::setw(8) << formid << "\"";
    ss << R"(,"found":)" << (form ? "true" : "false");
    if (form) {
        uint8_t form_type = mora_skyrim_runtime::rt::get_form_type(form);
