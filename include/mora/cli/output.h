...
public:
    ...
    void print_header(std::string_view version);
    ...
    void table(const std::vector<TableRow>& rows);

    // Box-drawing section header: ── title ──────
    void section_header(std::string_view title, size_t width = 54);

    // Inline stderr progress (carriage-return overwrite)
    void progress_update(std::string_view text);
    void progress_clear();
