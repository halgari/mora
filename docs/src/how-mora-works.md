...
That number is not a benchmark target — it is measured output from the logging in `plugin_entry.cpp`, captured via `QueryPerformanceCounter` on actual hardware. The runtime cost is not optimized — it is simply the cost of writing to memory sequentially.
...
A large Wabbajack modlist — 500 to 2,000 mods — can take five to ten minutes to reach the main menu. This is not because Skyrim's engine is slow. It is because dozens of independent runtime patchers are running sequentially, each doing the same categories of work:
...
Synthesis is the right idea — compute at build time, not runtime — applied through a framework that trades one set of complexity for another.
...
- Every patcher reimplements form lookup, memory layout knowledge, and field offsets — knowledge that is fragile across game updates.
...
Custom SKSE plugins are the right output format — they are native, fast, and integrate cleanly with the engine. The problem is the cost to produce them.
...
!!! note "What 'O(patches)' means in practice"
    200 weapon damage patches apply in under 1ms. 6,782 NPC name patches apply in 1.63ms. The runtime is sequential memory writes — as fast as a memory write can be. There is no smarter implementation because there is nothing to optimize: the work has already been done at compile time.
...
**Partial evaluation is well-studied.** The key property that makes Mora's compile-time evaluation tractable is that Datalog rules can be *partially evaluated* — executed against known facts at compile time, with residual unknowns deferred to runtime. This is not a hack; it is a formal technique with decades of research behind it. The compiler knows exactly which rules can be fully frozen and which cannot, without guessing.

**The data model is a natural fit.** ESP records are essentially database rows. Querying "all NPCs in BanditFaction with level >= 30 that do not already have SilverSword" is exactly the kind of join-and-filter query Datalog was designed for. You are not fighting the abstraction — you are working with it.
...
This is not a clever optimization. It is the correct architecture for the problem.
