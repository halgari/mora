...
`iron_weapons(Weapon):`
: The rule head. `iron_weapons` is the rule name. `Weapon` is a logic variable — it will be bound to each form that satisfies the clauses below. The colon opens the rule body.
...
`Type checking 1 rules`
: All clauses and effects have consistent types. `set_damage` expects a weapon form and an integer — both match.
...
`MoraRuntime.dll (16.5 KB)`
: The compiled output. A 16.5 KB native x86-64 DLL containing the pre-computed patches as direct memory writes. No rules are evaluated at runtime — only the results are baked in.
