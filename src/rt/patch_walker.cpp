...
static void apply_patch_entry(void* form, const PatchEntry& e,
    ...
    switch (e.field_id) {
        ...
        case fid(FieldId::Keywords): {
            if (e.value_type != static_cast<uint8_t>(mora::PatchValueType::FormID)) break;
            void* kw_form = mora_rt_lookup_form(static_cast<uint32_t>(e.value));
            if (!kw_form) break;
            if (e.op == fop(FieldOp::Add))
                mora_rt_add_keyword(form, kw_form);
            else if (e.op == fop(FieldOp::Remove))
                mora_rt_remove_keyword(form, kw_form);
            break;
        }
        case fid(FieldId::Spells): {
            if (e.value_type != static_cast<uint8_t>(mora::PatchValueType::FormID)) break;
            void* spell_form = mora_rt_lookup_form(static_cast<uint32_t>(e.value));
            if (!spell_form) break;
            if (e.op == fop(FieldOp::Add))
                mora_rt_add_spell(form, spell_form);
            else if (e.op == fop(FieldOp::Remove))
                mora_rt_remove_spell(form, spell_form);
            break;
        }
        case fid(FieldId::Shouts): {
            if (e.value_type != static_cast<uint8_t>(mora::PatchValueType::FormID)) break;
            void* shout_form = mora_rt_lookup_form(static_cast<uint32_t>(e.value));
            if (shout_form)
                mora_rt_add_shout(form, shout_form);
            break;
        }
        case fid(FieldId::Perks): {
            if (e.value_type != static_cast<uint8_t>(mora::PatchValueType::FormID)) break;
            void* perk_form = mora_rt_lookup_form(static_cast<uint32_t>(e.value));
            if (perk_form)
                mora_rt_add_perk(form, perk_form);
            break;
        }
        case fid(FieldId::Factions): {
            if (e.value_type != static_cast<uint8_t>(mora::PatchValueType::FormID)) break;
            void* fac_form = mora_rt_lookup_form(static_cast<uint32_t>(e.value));
            if (fac_form)
                mora_rt_add_faction(form, fac_form);
            break;
        }
